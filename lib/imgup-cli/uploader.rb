...
module ImgupCli
  # Handles the entire upload flow: copy to temp, upload via OAuth,
  # fetch available sizes, and produce snippet output.
  class Uploader
    ...
    def upload_file(tmp_path)
      ...
      upload_consumer = OAuth::Consumer.new(@consumer_key, @consumer_secret, site: "#{uri.scheme}://#{uri.host}")
      upload_access   = OAuth::AccessToken.new(upload_consumer, @access_token, @access_token_secret)
